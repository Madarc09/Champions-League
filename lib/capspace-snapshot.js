...
const SNAPSHOT_KEY = "champions-league:capspace-open-source-snapshot:2026-27:v5-strict-roster-counts";
...
export async function getCapSpaceSalarySnapshot({ force = false, strict = false } = {}) {
  ...
  const cachedIsComplete = cached
    && cached.recordCount >= MIN_VALID_RECORDS
    && Number(cached.teamCount) === TEAMS.length
    && (!Array.isArray(cached.failedTeams) || cached.failedTeams.length === 0);

  if (!force && cachedIsComplete && snapshotIsFresh(cached)) {
  ...
  if (fresh.recordCount < MIN_VALID_RECORDS || fresh.teamCount !== TEAMS.length) {
    if (!strict && cachedIsComplete) {
      const stale = {
        ...cached,
        stale: true,
