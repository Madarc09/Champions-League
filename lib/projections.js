...
function expectedSkaterGames(player, history, ranks, context) {
  ...
  if (!games.length) {
    ...
    return clamp(Math.round(context?.player?.games || rookieBase), 20, 82);
  }
  ...
  return clamp(Math.round(estimate), 45, 82);
}
...
function safeStatProjection(raw, baseline, player) {
  ...
  return Math.max(0, Math.round(clamp(raw, baseline * 0.72, baseline * 1.32)));
}
