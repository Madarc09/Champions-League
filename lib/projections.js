...
function projectGoalie(player, nhlHistoryInput, advancedBundle, ranks, context) {
  ...
  const nhlShutoutRate = weightedRate(nhlHistory, "shutouts", 0.05);
  ...
  const shutouts = Math.max(0, Math.round(nhlShutoutRate * projectedGames * clamp(2 - environment.goalsAgainst, 0.9, 1.1)));
  ...
  const projection = {
    ...
    modelBreakdown: {
      ...
      shutouts: { candidates: { nhlHistory: round(nhlShutoutRate * projectedGames, 1) }, final: shutouts }
    },
    context: {
    ...
    statReasons: {
      ...
      shutouts: `Shutouts project from the three-season shutout rate, expected workload and team defensive environment: ${Number(player.shutouts || 0)} to ${shutouts}.`
    },
    sources: [
